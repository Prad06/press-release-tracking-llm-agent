...
from pr_flow_agents.graph.linker.prompts import LINKER_DECISION_PROMPT_TEMPLATE
from pr_flow_agents.graph.linker.prompts import LINKER_DECISION_REFINER_PROMPT_TEMPLATE
...
def _parse_event_date_for_rank(event_date: Optional[str]) -> Optional[datetime]:
    raw = (event_date or "").strip()
    if not raw:
        return None
    if re.match(r"^\d{4}-\d{2}-\d{2}$", raw):
        return datetime.fromisoformat(raw)
    q_match = re.match(r"^(\d{4})-Q([1-4])$", raw)
    if q_match:
        year = int(q_match.group(1))
        quarter = int(q_match.group(2))
        month = (quarter - 1) * 3 + 1
        return datetime(year=year, month=month, day=1)
    y_match = re.match(r"^(\d{4})$", raw)
    if y_match:
        return datetime(year=int(y_match.group(1)), month=1, day=1)
    return None
...
def _extract_event_payload(doc: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if isinstance(payload, dict):
        out = dict(payload)
        out["quality_flag"] = str(doc.get("quality_flag") or out.get("quality_flag") or "")
...
def _guess_thread_for_event(*, ticker: str, sector: Optional[str], event: Dict[str, Any]) -> Tuple[str, str]:
    entities = [str(x).strip() for x in (event.get("entities") or []) if str(x).strip()]
    event_type = str(event.get("event_type") or "").upper()
    claim = str(event.get("claim") or "")
    sector_key = _safe_lower(sector)

    if sector_key == "biotech":
        for entity in entities:
            if re.search(r"[A-Za-z]+-\d+", entity):
                return f"{ticker.lower()}::program::{_stable_thread_slug(entity)}", f"Program {entity}"
        for entity in entities:
            if re.search(r"(disease|syndrome|cancer|tumou?r|hemophilia|sickle|diabetes)", _safe_lower(entity)):
                return f"{ticker.lower()}::disease::{_stable_thread_slug(entity)}", f"Disease {entity}"
        return f"{ticker.lower()}::general", "General"

    if sector_key == "aviation":
        claim_l = _safe_lower(claim)
        for entity in entities:
            ent_l = _safe_lower(entity)
            if "route" in ent_l or "airport" in ent_l:
                return f"{ticker.lower()}::route::{_stable_thread_slug(entity)}", f"Route {entity}"
        if "route" in claim_l or "between " in claim_l:
            return f"{ticker.lower()}::route::general", "Route General"
        for entity in entities:
            ent_l = _safe_lower(entity)
            if any(token in ent_l for token in ("airbus", "boeing", "a220", "a320", "a321", "737", "787", "fleet")):
                return f"{ticker.lower()}::fleet::{_stable_thread_slug(entity)}", f"Fleet {entity}"
        if event_type == "FINANCIAL" and any(token in claim_l for token in ("guidance", "outlook", "forecast")):
            return f"{ticker.lower()}::guidance", "Guidance"
        return f"{ticker.lower()}::general", "General"
...
def _score_candidate(
    *,
    ...
    score = 0.0
    new_type = str(new_event.get("event_type") or "").upper()
    cand_type = str(candidate.get("event_type") or "").upper()
    if new_type and new_type == cand_type:
        score += 4.0
    score += _entity_jaccard(_entity_set(new_event), _entity_set(candidate)) * 3.0

    new_dt = _parse_event_date_for_rank(str(new_event.get("event_date") or ""))
    cand_dt = _parse_event_date_for_rank(str(candidate.get("event_date") or ""))
    if new_dt and cand_dt:
        days = abs((new_dt - cand_dt).days)
        if days <= 30:
            score += 3.0
        elif days <= 60:
            score += 2.0
        elif new_dt.year == cand_dt.year and new_dt.month == cand_dt.month:
            score += 2.0

    if str(candidate.get("thread_id") or "") == provisional_thread_id:
        score += 2.0
    return score
...
def _build_scratchpad_text(doc: Optional[Dict[str, Any]]) -> str:
    ...
    claims = [str(x).strip() for x in (doc.get("latest_claims") or []) if str(x).strip()]
    if claims:
...
def _normalize_decision(*, raw: Any, new_event_id: str, default_thread_id: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "target_linked_event_id": str(payload.get("target_linked_event_id") or "").strip() or None,
        "thread_id": str(payload.get("thread_id") or default_thread_id).strip() or default_thread_id,
...
def load_silver_events(state: LinkerState) -> LinkerState:
    ...
    if not ticker or not press_release_id:
        logger.warning("linker_load_silver_events_missing_input ticker=%s press_release_id=%s", ticker, press_release_id)
        return {**state, "status": "SKIPPED", "error": "ticker and press_release_id are required"}
...
def prepare_current_event(state: LinkerState) -> LinkerState:
    ...
    return {
        **state,
        ...
        "provisional_thread_name": str((scratchpad_doc or {}).get("thread_name") or thread_name),
...
def retrieve_candidates(state: LinkerState) -> LinkerState:
    ...
    provisional_thread_id = str(state.get("provisional_thread_id") or "")
    ...
    for cand in candidates:
        score = _score_candidate(new_event=event, candidate=cand, provisional_thread_id=provisional_thread_id)
        ranked.append((score, cand))
    ranked.sort(
        key=lambda row: (row[0], str(row[1].get("updated_at") or ""), str(row[1].get("linked_event_id") or "")),
        reverse=True,
    )
    top = [cand for _, cand in ranked[:TOP_K]]
    logger.info(
...
def decide_action(state: LinkerState) -> LinkerState:
            ...
            [
                {
                    ...
                    "status": c.get("status"),
                    "supporting_silver_event_ids": c.get("supporting_silver_event_ids") or [],
                }
                for c in candidates
            ],
            ensure_ascii=True,
    ...
    try:
        ...
        logger.warning("linker_decision_failed silver_event_id=%s error=%s", silver_event_id, exc)
        raw = {
            "action": "NEW",
            "new_event_id": silver_event_id,
    ...
    decision = _normalize_decision(raw=raw, new_event_id=silver_event_id, default_thread_id=default_thread_id)
    logger.info(
...
def refine_decision(state: LinkerState) -> LinkerState:
    ...
    prompt = LINKER_DECISION_REFINER_PROMPT_TEMPLATE.format(
        ...
        new_event=json.dumps(silver_event, ensure_ascii=True),
        existing_thread_ids="\n".join(f"- {t}" for t in existing_tids) if existing_tids else "(none yet)",
        candidates=json.dumps(
            [
                {
                    "linked_event_id": c.get("linked_event_id"),
    ...
    try:
        ...
        refined = _normalize_decision(
            raw=raw,
            new_event_id=silver_event_id,
            default_thread_id=str(initial_decision.get("thread_id") or state.get("provisional_thread_id") or f"{state.get('ticker')}:General"),
        )
        logger.info(
        ...
        logger.warning("linker_refine_decision_failed silver_event_id=%s error=%s", silver_event_id, exc)
        return state
...
def apply_decision(state: LinkerState) -> LinkerState:
    ...
    thread_id = str(decision.get("thread_id") or state.get("provisional_thread_id") or f"{ticker}:General")
    ...
    if action == "NEW":
        ...
        applied["applied"] = _linked_store.append_supporting_silver(str(target_id), silver_event_id)
    elif action == "UPDATE":
        ...
        _linked_store.mark_superseded(old_linked_event_id=str(target_id), new_linked_event_id=new_id)
    ...
    impacted[str(applied.get("thread_id") or thread_id)] = str(state.get("provisional_thread_name") or "General")
    ...
    if action == "NEW":
        next_state["linked_events_created"] = int(state.get("linked_events_created") or 0) + 1
    elif action == "DUPLICATE":
        next_state["linked_events_duplicates"] = int(state.get("linked_events_duplicates") or 0) + 1
    elif action == "UPDATE":
        next_state["linked_events_updated"] = int(state.get("linked_events_updated") or 0) + 1
    elif action == "RETRACT":
        next_state["linked_events_retracted"] = int(state.get("linked_events_retracted") or 0) + 1
    return next_state
...
def refresh_scratchpads(state: LinkerState) -> LinkerState:
    ...
    for thread_id, thread_name in impacted.items():
        latest = _linked_store.list_by_thread(ticker=ticker, thread_id=thread_id, statuses=("ACTIVE",), limit=10)
        if not latest:
            latest = _linked_store.list_by_thread(ticker=ticker, thread_id=thread_id, statuses=ACTIVE_STATUSES, limit=10)
        latest_ids = [str(x.get("linked_event_id") or "") for x in latest if str(x.get("linked_event_id") or "").strip()]
        latest_claims = [str(x.get("canonical_claim") or "").strip() for x in latest[:10] if str(x.get("canonical_claim") or "").strip()]
    ...
    logger.info("linker_refresh_scratchpads_done ticker=%s impacted_threads=%s", ticker, len(impacted))
    return state
...
def finalize_output(state: LinkerState) -> LinkerState:
    ...
    if _mlflow_enabled():
        ...
        mlflow.log_metric("linker_processed_silver_events_count", float(result["processed_silver_events_count"]))
        mlflow.log_metric("linker_linked_events_created", float(result["linked_events_created"]))
        mlflow.log_metric("linker_linked_events_duplicates", float(result["linked_events_duplicates"]))
        mlflow.log_metric("linker_linked_events_updated", float(result["linked_events_updated"]))
        mlflow.log_metric("linker_linked_events_retracted", float(result["linked_events_retracted"]))
        mlflow.log_metric("linker_impacted_threads_count", float(result["impacted_threads_count"]))
